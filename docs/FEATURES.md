...
**사용 방법**:
1. 탭 상단의 손가락 아이콘(드래그 모드) 클릭하여 드래그 모드 활성화
2. 블록을 클릭하고 드래그
3. 원하는 레벨의 드롭 영역에 드롭
4. 삽입 인디케이터로 드롭 위치 확인 가능
5. 자동으로 레벨 및 순서 업데이트
...
**드래그 모드 특징**:
- 드래그 모드 활성화 시 블록이 드래그 가능 상태로 변경
- 드래그 중인 블록은 반투명 처리
- 드롭 위치에 삽입 인디케이터 표시 (세로/가로)
- 같은 레벨 내 순서 변경 가능
- 다른 레벨로 이동 시 자동 순서 조정
- 다중 행 레이아웃 지원 (Y 좌표 기반 같은 줄 판단)

**기술적 세부사항**:
- HTML5 Drag and Drop API 사용
- 마우스 위치 기반 정확한 삽입 위치 계산
- 블록의 왼쪽 경계를 고려한 삽입 위치 판단
- X/Y 좌표 기반 다중 행 레이아웃 지원
...
### 모드 전환
- **보기 모드** (기본): 블록 클릭 시 편집 모달 열림
- **드래그 모드**: 손가락 아이콘 클릭으로 활성화, 블록 드래그 앤 드롭 가능
- **연결선 모드**: 연결 아이콘 클릭으로 활성화, 블록 간 의존성 연결 가능
